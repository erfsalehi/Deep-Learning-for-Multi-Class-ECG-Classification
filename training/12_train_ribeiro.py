...
from src.models.ribeiro_cnn import build_ribeiro_resnet
...
def train_ribeiro():
    ...
    print("Loading PTB-XL data for Ribeiro Baseline...")
    ...
    # Fold 10 is test, 9 is val, 1-8 is train
    ...
    model = build_ribeiro_resnet(input_shape=(1000, 12), num_classes=5)
    model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=1e-3),
                  loss='binary_crossentropy', 
                  metrics=['accuracy', tf.keras.metrics.AUC(name='auc', multi_label=True)])
    
    print("Starting Ribeiro ResNet training...")
    history = model.fit(
        train_gen,
        validation_data=val_gen,
        epochs=EPOCHS,
        callbacks=[
            tf.keras.callbacks.ModelCheckpoint(os.path.join(OUTPUT_DIR, 'ribeiro_resnet_best.keras'), save_best_only=True),
    ...
    plt.legend(); plt.title('Ribeiro ResNet Loss')
    
    plt.subplot(1, 2, 2)
    plt.plot(history.history['auc'], label='Train AUC')
    plt.plot(history.history['val_auc'], label='Val AUC')
    plt.legend(); plt.title('Ribeiro ResNet AUC')
    
    os.makedirs('results/figures/baselines/', exist_ok=True)
    plt.savefig('results/figures/baselines/ribeiro_training_history.png')
    print("Completed. Saved to", os.path.join(OUTPUT_DIR, 'ribeiro_resnet_best.keras'))
