...
from src.models.transformer_ecg import build_transformer_ecg
...
def train_transformer():
    ...
    print("Loading PTB-XL data for Transformer Baseline...")
    ...
    model = build_transformer_ecg(input_shape=(1000, 12), num_classes=5)
    ...
    history = model.fit(
        ...
        epochs=EPOCHS,
        callbacks=[
            tf.keras.callbacks.ModelCheckpoint(os.path.join(OUTPUT_DIR, 'transformer_best.keras'), save_best_only=True),
    ...
    plt.legend(); plt.title('Transformer AUC')
    
    os.makedirs('results/figures/baselines/', exist_ok=True)
    plt.savefig('results/figures/baselines/transformer_training_history.png')
    print("Completed. Saved to", os.path.join(OUTPUT_DIR, 'transformer_best.keras'))
